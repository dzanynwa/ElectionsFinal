...
<?php 
               ...
               $id = $_GET['id'];
                ...
                $drugi = mysqli_query($mysqli, "SELECT COUNT(DISTINCT(Unit_ID)) as total2 FROM ElectionUnit e WHERE (e.Parent_ID < 211 AND e.Parent_ID > 200)"); 
                $row2 = mysqli_fetch_assoc($drugi);
                $treci = mysqli_query($mysqli, "SELECT COUNT(DISTINCT(Candidate)) as total3 FROM Candidate c, Votes v WHERE v.Position = 6 AND c.Candidate_ID = v.Candidate");
